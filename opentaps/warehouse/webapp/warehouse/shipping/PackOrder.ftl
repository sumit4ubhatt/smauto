...
<script type="text/javascript">
	function creditCheck(){
	  ...
	  alert('${creditLimit}');
		if (parseInt('${creditLimit}') > parseInt('${orderHeader.grandTotal}')){
			alert('inside if' + '${orderHeader.grandTotal}');
			alert('you can ship this');
            return true;
        }else{
        	alert('inside else' + '${orderHeader.grandTotal}');
        	alert('you cannot ship this');
        	return false;
        }
	  return false;
	}
...
<#if warehouseSecurity.hasFacilityPermission("WRHS_SHIP_PACK")>
    ...
    Please do not ship more than ${creditLimit?if_exists}
    <div class="screenlet">
  <div class="screenlet-body">
    ...
    <#if showInput != "N" && orderHeader?exists && orderHeader?has_content>
      ...
      <#if showInput != "N" && hideGrid != "Y" && itemInfos?has_content && (totalAvailableToPack gt 0)>
        ...
        <form name="multiPackForm" method="post" action="<@ofbizUrl>ProcessBulkPackOrder</@ofbizUrl>" style="margin: 0;" onsubmit="return creditCheck();">
          <input type="hidden" name="facilityId" value="${facilityId?if_exists}">
          <input type="hidden" name="orderId" value="${orderId?if_exists}">
          <input type="hidden" name="shipGroupSeqId" value="${shipGroupSeqId?if_exists}">
